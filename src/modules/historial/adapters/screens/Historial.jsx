...
export default function Historial() {
  ...
  const historialStatic = [
    ...
    // Agrega más restaurantes estáticos si es necesario
  ];
  const [historial, setHistorial] = useState(historialStatic);
  ...
  return (
    <View style={styles.container}>
      <FlatList
        data={historial}
        renderItem={({ item }) =>
          <FlatListHistorial image={item.image} price={item.price} description={item.description} status={item.status} date={item.date} />
        }
